<?php
	...
	getPageHeader("Manage Divisions");
	closePageHeader();
	getMenuLinks(5);
?>
		  ...
		  <div class="row">
			...
			<div class="col-md-12">
				...
				<form id="new-record" data-parsley-validate="true" action="controller.php" method="post">
					...
					<input type="hidden" name="action" value="new-division" />
				</form>
...
<?
	$sect = getSectors();
	$acctopt = "<select name='sectors[]' class='form-control col-md-7 col-xs-12' data-parsley-required='true'><option value=''>Select A Sector</option>";
	if($sect->num_rows > 0){
		while($rowz = $sect->fetch_assoc()) {		
			$acctopt .="<option value='".$rowz['id']."'>".$rowz['caption']."</option>";
		}
	}
	$acctopt .="</select>";
?>
